...
func analyzeChunk(data []byte, begin, end int) *flatMap {
	...
	for i := begin; i < end; {
		...
		m.add(data[channelBegin:channelBegin+int(channelLen)], channelHash, month, messageLength, stampCount)
		i = p
	}
	return m
}
...
func newFlatMap(capacity int) *flatMap {
	...
	return &flatMap{
		keys:    make([]byte, 0, c*8),
		entries: make([]mapEntry, c),
		aggs:    make([]channelAgg, 0, c/2),
	}
}

func (m *flatMap) add(key []byte, hash uint32, month int, messageLength, stampCount uint32) {
...
func (m *flatMap) findOrInsert(key []byte, hash uint32) *mapEntry {
...
func (m *flatMap) mergeFrom(other *flatMap) {
	for i := range other.entries {
		...
		key := other.keys[e.pos : e.pos+uint64(e.len)]
		dst := m.findOrInsert(key, hashBytes(key))
...
func writeResult(w io.Writer, m *flatMap) error {
	...
	sort.Slice(entries, func(i, j int) bool {
		...
		return bytes.Compare(m.keys[a.pos:a.pos+uint64(a.len)], m.keys[b.pos:b.pos+uint64(b.len)]) < 0
	})
	...
	for i := range entries {
		...
		for month := range agg.months {
			...
			line = line[:0]
			line = append(line, m.keys[e.pos:e.pos+uint64(e.len)]...)
			line = append(line, ',')
