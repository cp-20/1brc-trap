...
unsafe class Program
{
    ...
    struct Stats { public uint TotalLen, Stamps, Messages; public ushort MinLen, MaxLen; }
    // Keeping the full hash made Entry 16B and was slower than this 12B tag layout on 10M rows.
    struct Entry { public int Pos, Len; public ushort Id, Tag; }
    sealed class FlatMap
    {
        public readonly Entry[] Entries = new Entry[Capacity];
        public readonly Stats[] Aggs = new Stats[(Capacity / 2) * 12];
        // Fixed public/private keys use at most 241KB; raise this 1MB ceiling if channel generation grows.
        readonly byte[] keyStorage = GC.AllocateUninitializedArray<byte>(Capacity * 32, pinned: true);
        readonly nint keys;
        int keyBytes;
        public int Size;
        public FlatMap() { keys = (nint)Unsafe.AsPointer(ref MemoryMarshal.GetArrayDataReference(keyStorage)); }
        int Find(byte* key, int len, uint hash)
        {
            ...
            for (; ; i = (i + 1) & (Capacity - 1))
            {
                ...
                if (e.Len == 0) { new ReadOnlySpan<byte>(key, len).CopyTo(new Span<byte>((byte*)keys + keyBytes, len)); e = new Entry { Pos = keyBytes, Len = len, Id = (ushort)Size++, Tag = tag }; keyBytes += len; return i; }
                if (e.Tag == tag && e.Len == len && Equal((byte*)keys + e.Pos, key, len)) return i;
            }
        }
        public void Add(nint data, long pos, int len, uint hash, int month, uint ml, uint stamps)
        {
            int slot = Find((byte*)data + pos, len, hash); ref Stats s = ref Aggs[Entries[slot].Id * 12 + month];
        ...
        public void Merge(FlatMap other)
        {
            foreach (ref readonly Entry a in other.Entries.AsSpan())
            {
                if (a.Len == 0) continue; byte* key = (byte*)other.keys + a.Pos; int slot = Find(key, a.Len, Hash(key, a.Len)); int di = Entries[slot].Id * 12, si = a.Id * 12;
        ...
        public byte* Key(int pos) => (byte*)keys + pos;
    ...
    // Manual first/last-word probes did not beat the intrinsic SequenceEqual on the 10M workload.
    ...
    // First/middle/last sampling changed the 10M mean by less than 1%; keep full CRC coverage.
    ...
    // Computing the day from only the first eight digits was 14% slower on the 100M workload.
