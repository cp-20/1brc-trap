...
// The former C++ map/vector abstraction gave analyze_worker a 304-byte frame
// and an 0x8b3-byte body. This POD/pthread translation emitted the same worker
// body as C at the 2.45-second checkpoint, so the final hot kernel stays
// intentionally C-shaped while remaining a standalone C++23 source file.
// Historical rejects in that superseded parser: a second independent
// two-digit branch lost, and one combined scalar numeric tail regressed EC2 1B
// from 3.007 to 3.038 s. The final shared VNNI path below is a different win.
// Likewise, FAST_BITS=17 was 3.61 s versus 3.60 s for 16 in the old two-table
// map; the final direct-ID design remeasured 17 as the optimum.
