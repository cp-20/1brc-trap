...
React製フロントエンド、Echo v5製Go API、非同期worker、隔離された計測runner、データ生成ツール、AWS CDK、Terraform、Ansibleを含みます。
...
mise exec -- bun run api:generate
...
- `apps/server`：Echo v5製Go API、worker、migration
- `apps/runner`：forced-command SSH gatewayとDocker計測処理
- `apps/mock-auth`：ローカル用認証proxy
- `packages/api`：TypeSpecのAPI定義、OpenAPI、生成したTypeScript型
...
TypeScriptの依存関係はBun workspaceと `bun.lock`、Goの依存関係はGo moduleと `go.sum` で管理します。
...
| profile  | `PROFILING_SECRET`（32文字以上の英数字・`_`・`-`）、任意で `PROFILING_PORT`（既定値 `6499`）                                      |
...
`PROFILING_SECRET` を設定すると、指定ポートでGoのpprof endpointが起動します。
NeoShowcaseでそのポートを公開し、`http://<公開ホスト>:<ポート>/<PROFILING_SECRET>/debug/pprof/` に接続します。
profileにはプロセスの内部情報が含まれるため、ログとポートの閲覧者を制限し、利用後は環境変数とポート公開を削除してください。
...
`packages/api/main.tsp` がHTTP API schemaのSSoTです。
生成コマンドはOpenAPI、Web用の `openapi-typescript` 型、Go用の `oapi-codegen` 型・server interface・Echo v5 routerを更新します。

```sh
mise exec -- bun run api:generate
mise exec -- bun run api:check
```

## Database migration

SQL migrationは `apps/server/migrations` に追加します。
API起動時のmigrationはMariaDBのadvisory lockを取得し、`onebrc_migrations` に未記録のファイルだけを適用します。
同じ処理はローカルで単独実行できます。

```sh
mise exec -- bun run db:check
