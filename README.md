...
`PROFILING_SECRET` を設定すると、指定ポートでBun Inspectorが起動します。
NeoShowcaseでそのポートを公開し、`https://debug.bun.sh/#<公開ホスト>:<ポート>/<PROFILING_SECRET>` を開いて接続します。
Inspectorは実行中プロセス内で任意コードを実行でき、起動ログにも接続URLが出るため、ログの閲覧者を制限し、利用後は環境変数とポート公開を削除してください。

APIは起動時に未適用のDrizzle migrationを実行し、worker loopを開始してからHTTP serverを起動します。
...
`apps/server/src/infrastructures/schema.ts` をschemaのSSoTとし、変更後はDrizzle Kitでmigrationを生成します。
API起動時のmigrationはMariaDBのadvisory lockを取得し、Drizzleの `__drizzle_migrations` に未記録のmigrationだけを適用します。
同じ処理はローカルで単独実行できます。

```sh
mise exec -- bun run db:generate
