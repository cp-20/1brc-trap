...
public:
  ...
  void merge(const FlatMap &other) {
    for (const Entry &source : other.entries_) {
      if (source.channel.empty())
        continue;
      Entry &target =
          find_or_insert(source.channel.data(), source.channel.size());
      for (int month = 0; month < 12; ++month) {
        const Stats &a = source.months[month];
        if (a.messages == 0)
          continue;
        Stats &b = target.months[month];
        if (b.messages == 0) {
          b = a;
        } else {
          b.min_length = std::min(b.min_length, a.min_length);
          b.max_length = std::max(b.max_length, a.max_length);
...
FlatMap analyze(const char *cursor, const char *end) {
  FlatMap stats;
  while (cursor < end) {
...
int main(int argc, char **argv) {
  ...
  const char *begin = std::find(input.data, end, '\n') + 1;
  std::array<const char *, THREADS + 1> boundary{};
  boundary[0] = begin;
  boundary[THREADS] = end;
  for (int i = 1; i < THREADS; ++i) {
    const char *middle = begin + (end - begin) * i / THREADS;
    boundary[i] = std::find(middle, end, '\n') + 1;
  }
  ...
  for (int i = 0; i < THREADS; ++i) {
    workers.emplace_back(
        [&, i] { local[i] = analyze(boundary[i], boundary[i + 1]); });
  }
